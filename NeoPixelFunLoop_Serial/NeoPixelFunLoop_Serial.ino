...
const uint16_t AnimCount = PixelCount / 5 * 2 + 10; // we only need enough animations for the tail and one extra

uint16_t PixelFadeDuration = 3000; // third of a second
// one second divide by the number of pixels = loop once a second
uint16_t NextPixelMoveDuration = 6000 / PixelCount; // how fast we move through the pixels
...
void LoopAnimUpdate(const AnimationParam& param)
{
    ...
    if (param.state == AnimationState_Completed)
    {
        ...
        if (frontPixel == 0)
        {
            ...
            frontColor = colorGamma.Correct(frontColor);
        }
        uint16_t indexAnim;
...
uint32_t lasttime = millis(), interval;
...
void loop()
{
    ...
    if(now - lasttime > interval){
      lasttime = now;
      interval = random(20, 100)*100;
      uint16_t newDur = random(2000/PixelCount,10000/PixelCount);
      animations.ChangeAnimationDuration(0, newDur);
      PixelFadeDuration = random(newDur*6, newDur*10);
    }
}
