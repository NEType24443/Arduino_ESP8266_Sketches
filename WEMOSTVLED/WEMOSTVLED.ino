...
void rainbow(int wait , int cycles) {
  ...
  unsigned long begining;
  while(cycles>0){// means we'll make 3*65536/256 = 768 passes through this outer loop:
...
void looper(uint32_t color, uint16_t wait, uint8_t cycles, uint8_t numPix){
  unsigned long begining ;
  strip.clear();
  while(cycles>0){
    for(uint8_t i=0; i<PIXELS; i++){
      for(int n=numPix; n>0; n--){
        strip.setPixelColor(i+(n-1)*PIXELS/numPix, color);
      }
      strip.show();
      begining = millis();
      while(begining+wait>=millis()){
      ...
      for(int n=numPix; n>0; n--){
        strip.setPixelColor(i+(n-1)*PIXELS/numPix, strip.Color( 0, 0, 0));
      }
      strip.show();       
    }
    cycles--;
...
void reflector(uint32_t color, uint32_t cycdur, uint8_t cycles, uint32_t d1, uint32_t d2, uint32_t d3, uint32_t d4){
  ...
  if((d1>0)||(d2>0)||(d3>0)||(d4>0)){
    while(cycles>0){
      ...
      tim = millis();
      if((ini1+d1<=tim) && (d1>0)){
        pixelreflect(pac1, color);
        //Serial.print("Called 1");
        ini1=millis();
      }
      if((ini2+d2<=tim) && (d2>0)){
        pixelreflect(pac2, color);  
        //Serial.print("Called 2");
        ini2=millis();
      }
      if((ini3+d3<=tim) && (d3>0)){
        pixelreflect(pac3, color);
        //Serial.print("Called 3");
        ini3=millis();
      }
      if((ini4+d4<=tim) && (d4>0)){
        pixelreflect(pac4, color);
...
void pixelreflect( int* pixel, uint32_t color){
...
void loading(uint32_t color, uint8_t cycles, uint32_t dur){
  strip.clear();
  strip.show();
  unsigned long begining = millis(),tim1 = begining,tim2 = begining;
  uint16_t pixellead = PIXELS/4, pixellag = 0;
  uint16_t duron = delayMod( begining, dur), duroff = duron + 100;
  for(uint8_t i=0; i<=pixellead; i++){
    strip.setPixelColor(i, color);
  }
  strip.show();
  while(cycles){
    if(begining>=tim1+duron){
      pixellead = fillLoop( pixellead, color);
      if(pixellead == 0){
        Serial.print("Cycles: ");
        Serial.print(cycles);
        cycles--;
      ...
      Serial.print("\nDuron: ");
      Serial.print(duron);
      duron = delayMod( begining, dur);
    }
    if(begining>=tim2+duroff){
      pixellag = fillLoop( pixellag, strip.Color( 0, 0, 0));
      Serial.print("\nDuroff: ");
      Serial.println(duroff);
      duroff = delayMod( begining, dur);
      tim2=millis();
    }
    begining=millis();
    yield();
...
uint16_t delayMod( int tim, int dur) {
  dur += 50*sin(tim);
  return(dur);
}

uint8_t fillLoop( int pixel, uint32_t color){
...
void siteLoading(){
  File file = SPIFFS.open("/loading.html","r");
  server.streamFile(file, "text/html");
  file.close();
  if(server.hasArg("duration")){
    String hex = server.arg("color"); 
    uint16_t dur = server.arg("duration").toInt();
    uint8_t cycle = server.arg("cycle").toInt();
    uint32_t color = hextocolor(hex);
    loading(color, cycle, dur);
  }
}
